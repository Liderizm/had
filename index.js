...
.setThumbnail()
.addField(`VR-AT | Teşekkürler`, `**Selamlar, Ben VR-AT (VR-AT'nin Geliştiricisi) Öncelikle Botumuzu Eklediğiniz ve Bize Destek Olduğunuz İçin Sizlere Teşekkürlerimi Sunarım**`)
.addField(`Alone | Prefix`, `**VR-AT Botun Prefixi(ön eki) = \`${fynx.prefix}\`\n\n Değiştirebilmek için \`${fynx.prefix}prefix\` Yazabilirsiniz.**`)
.addField(`VR-AT | Nasıl Kullanılır?`, `**VR-AT botun tüm özelliklerinden yararlanabilmek için sadece \`${fynx.prefix}yardım\` yazmanız yeterlidir.**`)
.addField(`VR-AT | Linkler`, `**Sohbet Kanalına -davet Yazmanız Yeterlidir**`)
.setFooter(`VR-AT | Gelişmiş Türkçe Bot | 2020`)
.setTimestamp();
...
.setDescription(`<:a_:821738957997211659>  <@!${message.author.id}> **adlı kullanıcı tarafından** <#${message.channel.id}> **kanalına gönderilen mesaj silindi!** \n\nSilinen Mesaj: **${message.content}**`)
  .setFooter("VR-AT Bot | Log Sistemi")
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                ...
                .setFooter(`VR-AT Bot | Log Sistemi Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            };
